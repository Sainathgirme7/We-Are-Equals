...
function Convert() {
  ...
  const sign = (inputRef) => {
    ...
    for (let word of strWords) {
      if (words.wordAnimations[word]) {
        ref.animations.push(['add-text', word + ' ']);
        words.wordAnimations[word](ref);
      } else {
      ...
      <div className='row'>
        <div className='col-md-3'>
          ...
          <div className='space-between'>
            <button className="button-style w-33" onClick={startListening}>
              Mic On <i className="fa fa-microphone"/>
            </button>
            <button className="button-style w-33" onClick={stopListening}>
              Mic Off <i className="fa fa-microphone-slash"/>
            </button>
            <button className="button-style w-33" onClick={resetTranscript}>
              Clear
            </button>
          </div>
          <textarea rows={3} ref={textFromAudio} value={transcript} placeholder='Speech input ...' className='w-100 input-style' />
          <button onClick={() => sign(textFromAudio)} className='button-style w-100 btn-start'>
            Start Animations
          </button>
          <label className='label-style'>
            Text Input
          ...
          <button onClick={() => sign(textFromInput)} className='button-style w-100 btn-start'>
            Start Animations
          </button>
        </div>
